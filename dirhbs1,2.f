...
c
     ...
     &                tt(ntx)
      ...
      common /optopt/ itx,icm,icou,ipc,inl,idd
      common /pair  / del(2),spk(2),spk0(2)
      common /tapes / l6,lin,lou,lwin,lwou,lplo,laka,lvpp,wf
      common /textex/ tp(2),tis(2),tit(2),tl(0:30)
...
c---- loop of the j-blocks
         ...
         do ib = 1,nb
            ...
            !write(l6,*)'lf es ', lf
            !write(l6,*)'lg es ', lg
            nf  = id(ib,1) !amount of element on nf
            ng  = id(ib,2) !amount of element on ng
            !write(l6,*)'lf es ', lf
            !write(l6,*)'lg es ', lg
            nh  = nf + ng
            nhfb= nh + nh
...
C####
            !open(l6,file='dirhbwf.out',status='unknown')
c------- wave functions in the coordinate space    
            k1=kacan(ib,it)+1
            
            k2=kacan(ib,it)+kdcan(ib,it)
            do k = k1,k2
               e1 = eecan(k,it) !energy
              ! write(l6,*)e1
               !write(l6,*)' j =', 2*ijb(ib)-1,'/2',  e1
  !104          format(' j =',i2,'/2',a1,f6.2)
               do ih=1,ngh
                  !write(l6,*)'k1 es ', k1 
                  !write(l6,*)'k2 es ', k2
                  !write(l6,*)'ih es ', ih*b0
                  ...
                  !write(l6,*)'wave function nf :',ih ,  s/wx, sd/wxd
                  !write(l6,*)'-------------------------'
               enddo  
               suml=zero
               sums=zero
               ...
               do ih=1,ngh2
                  ...
                  !write(l6,*)'wave function nf :',ih ,  wave, dwave
                 
               enddo
               !write(l6,*)'-------------------------'
            enddo
            !close(lin)
c------- print wave functions and energies
       !     if (ib.eq.1) e0 = h(ng+1)
        !    k1 = kacan(ib,it)+1
         !   k2 = kacan(ib,it)+kdcan(ib,it)
         !   do k = k1,k2
          !     e1 = eecan(k,it) !energy 
         !      v1 = vvcan(k,it) !occupation number
          !     d1 = decan(k,it) 
           !    write(l6,*)e1
               ! gap
c           search for the main oscillator component
              ! do ih=1,ngh
               !   wave(ih,k,it)
                !  dwave(ih,k,it)
                !  write(l6,*)'wave function nf :',ih , i,  wave, dwave
               !enddo
...
c         if (lpr) write(l6,105) tit(it)
c  105    format(' anti-particle energies and gaps ',1x,
...
c------- Represent the integral Rn*RN as a matrix element of the potential
...
c
     ...
     &                c_s,c_v,c_ts,c_tv,d_s,d_v,d_ts,d_tv,dsat
      ...
      common /eeecan/ eecan(nkx,4),decan(nkx,4),vvcan(nkx,4),
     &                fgcan(nhx,nkx,4),mulcan(nkx,4),fgcan1(nhx,nkx,4)
      common /blocan/ kacan(nbx,4),kdcan(nbx,4),nkcan(2)
      common /nucnuc/ amas,nama,nneu,npro,nucnam
      common /bloqub/ ijb(nbx),ilb(nbx,2),ipb(nbx),ikb(nbx)      
      common /blodir/ ka(nbx,4),kd(nbx,4)
...
c 
      data ash/100.d0/ 
      if(lpr)
     &write(l6,*) ' ****** BEGIN CANON-WAVEFT*************************'
      r = zero
      !dimension rnl2(nrx,2)
c
c
c---- BCS structure in the canonical basis:
c     
     ...
     &           '  *       ',
     &           nucnam,nama,'  N = ',nama - npro,'  Z = ',npro
            write(l6,'(5f10.4)') amu, amsig, amome, amdel, amrho
            write(l6,'(4f10.4)') gsig, ggome, gdel, grho
      do it = 1,2
         write(l6,100) tit(it)
  100    format(' single-particle energies and gaps ',1x,
     &          'in the canonical basis: ',a,/,1x,66(1h-))
...
c------- reordering according to the energy h(k) 
            ...
            do k=1,nf
               ...
               mulcan(klp,it)=mul
               do i=1,nh
                  fgcan1(i,klp,it)=dd(i+(ng+k-1)*nh)
               enddo
            enddo           
            kdcan(ib,it) = klp - kacan(ib,it)
            ...
            do k=1,ng
               ...
               do i=1,nh
                  !write(l6,*)'i',i
                  fgcan1(i,kla,it+2)=dd(i+(k-1)*nh)
                  !write(l6,*)'fgcan1',fgcan1(i,kla,it+2)
...
c------- wave functions in the coordinate space    
            ...
            !k1 = ia(ib,it)+1
            !k2 = k1+nf-1
            !write(l6,*)'k1',k1,'k2' , k2
            do k = k1,k2
               !write(l6,*)'k1',k1,'k2' , k2
               ipar = ipb(ib)
               !do i=1,nh
                  !write(l6,*)'k',k
                  !write(l6,*)'fgcan1',k , fgcan1(i,k,it)
                  !write(l6,*)'fgcan1',fgcan1(i,kla,it+2)
                  
               !enddo
               
c               if (eecan(k,it).gt.25.)  goto 20
               e1 = eecan(k,it) !energy
                  ! write(l6,*)e1
               write(l6,104) 2*ijb(ib)-1, tp(ipar), e1
104            format(' j =',i2,'/2',a1, f12.4)
   
               call plotw(it,ib,k,.true.)
               !write(l6,*) 'NG',ng,'NF' ,nf,  'K', k, 'lf' ,lf,'lg',lg
                  !write(l6,*)'ib es ', ib
               
              ! do ih=1, mxpl + 0.5*mxpl   
                 ! call rwave(it,ib,k,r,f,g,ih)
                  !write(l6,'(f10.3,2f15.6)') r, f, g
                   !write(l6,'(i, f10.3,2f15.6)')lf, lg, r, f, g
                 ! r = r + 0.100
               
C   20          continue   
              ! enddo !wf
c               do ih=1,ai
c                  wx = xh(ih)*sqrt(wh(ih))*b0**(3./2.)*sqrt(4*pi)
c                  wxd= wx*b0
c                  s = 0.0
c                  sd = 0.0
c                  do j = 1, nf    !loop over oscilator states
c                     s = s + fgcan(j,k,it)*rnl(j,lf,ih)
c                     sd = sd + fgcan(j,k,it)*rnl1(j,lf,ih)
c                  enddo                                     
c                  wave(ih,k,it) = s/wx
c                  dwave(ih,k,it) = sd/wxd
c                  s = 0.0
c                  sd = 0.0
c                  do j = 1, ng
c                     s = s + fgcan(nf+j,k,it)*rnl(j,lg,ih)
c                     sd = sd + fgcan(nf+j,k,it)*rnl1(j,lg,ih)
c                  enddo
c                  wave(ai+ih,k,it) = s/wx
c                  dwave(ai+ih,k,it) = sd/wxd 
c                  write(l6,'(f10.3,2f15.6)') r, wave(ih,k,it),
c     &                  wave(ai+ih,k,it)
c                  r = r + 0.100
c               
c               enddo 
...
c               do ih=1,ngh
c                  wx = wdcor(ih)
c                  suml=suml+wave(ih,k,it)**2
c                  sums=sums+wave(ngh+ih,k,it)**2
c               enddo
c               snorm = one/sqrt(suml+sums)
c               do ih=1,ngh2
c                  wave(ih,k,it) = wave(ih,k,it)*snorm
c                  dwave(ih,k,it) = dwave(ih,k,it)*snorm
...
c======================================================================c
      ...
      subroutine rwave(it,ib,k,az,r,f,g,ih)
...
c
      ...
      lf  = ilb(ib,1)
      lg  = ilb(ibg,2)
      !write(l6,*) 'NG',ng,'NF' ,nf,nd,'lf',lf,'lg',lg
C---------------------
      !k1  = k
C---------------------
c     
...
c
      ...
      f  = b0**(-1.5d0)
      !write(l6,*) x, rr, f
      rnl2(1,1) = sq(2)*f*wgi(lf+1)*x**lf*exp(-half*rr)
      rnl2(2,1) = rnl2(1,1)*(lf+1.5d0-rr)*shi(lf+1)
      rnl2(1,2) = sq(2)*f*wgi(lg+1)*x**lg*exp(-half*rr)
      rnl2(2,2) = rnl2(1,2)*(lg+1.5d0-rr)*shi(lg+1)
      do n = 3,nrm
         rnl2(n,1)  = ((2*n+lf-2.5d0-rr)*rnl2(n-1,1) -
     &           sq(n-2)*sqh(n-2+lf)*rnl2(n-2,1))*sqi(n-1)*shi(n-1+lf)
         rnl2(n,2)  = ((2*n+lg-2.5d0-rr)*rnl2(n-1,2) -
     &           sq(n-2)*sqh(n-2+lg)*rnl2(n-2,2))*sqi(n-1)*shi(n-1+lg)
...
c------------------------------------
c        the functions rnl,rnl1 contain already the measure
C         u1 =  f*exp(-half*rr)*sqrt(wh(ih)*rr)
c------------------------------------
c
c------- basis for fermions
C         do l = 0,nlm
C            rnl(1,l,ih)  = u1
c            rnl(2,l,ih)  = u1*(l+1.5d0-rr)*shi(l+1)
c            u1           = u1*r*shi(l+1)
c            rnl1(1,l,ih) =    (l-rr)*rnl(1,l,ih)*ri
c            rnl1(2,l,ih) = ((2+l-rr)*rnl(2,l,ih) -
c     &                       2*sqh(l+1)*rnl(1,l,ih))*ri
c
c            do n = 3,nrm
c               rnl(n,l,ih)  = ((2*n+l-2.5d0-rr)*rnl(n-1,l,ih) -
c     &           csq(n-2)*sqh(n-2+l)*rnl(n-2,l,ih))*sqi(n-1)*shi(n-1+l)
c               rnl1(n,l,ih) = ((2*n+l-2-rr)*rnl(n,l,ih) -
c     &           2*sq(n-1)*sqh(n-1+l)*rnl(n-1,l,ih))*ri
c            enddo
c         enddo
c
c------- basis for bosons
c         rnb(1,ih)  = fe
c         rnb(2,ih)  = fe*(1.5d0-rr)*shi(1)
c         do n = 3,no
c            rnb(n,ih) = ((2*n-2.5d0-rr)*rnb(n-1,ih) -
c     &           sq(n-2)*sqh(n-2)*rnb(n-2,ih))*sqi(n-1)*shi(n-1)
c         enddo
c
c      enddo
c
...
C------------------
      ...
      do n = 1,nf
         !
         sf = sf + fgcan(n,mf,it)*rnl2(n,1)
         !write(l6,*)n,mf, fgcan(n,mf,it)
         !write(l6,*)'------------------------------'
C------------------
         !snf = snf + fgcan(n+(k-1)*nd,mf,it)*fgcan(n+(k-1)*nd,mf,it)
         if (imc.eq.0 .and. iplot.eq.1) then
         write(l6,'(f15.6)') fgcan(n+(k-1)*nd,mf,it)
         end if
C------------------
      enddo
      if (imc.eq.0 .and. iplot.eq.1) then
      ...
      do n = 1,ng
         sg = sg + fgcan(nf+n,mf,it)*rnl2(n,2)
         !write(l6,*)nf+n,mf , fgcan(nf+n,mf,it)
         !write(l6,*)'-----------------------------'
C------------------
cll         sng = sng + fg(nf+n+(k-1)*nd,mf)*rnl(n,2)
         !sng = sng + fgcan(nf+n+(k-1)*nd,mf,it)*fgcan(nf+n+(k-1)*nd,mf,it)
         if (imc.eq.0 .and. iplot.eq.1) then
            !write(l6,'(f15.6)') fg(nf+n+(k-1)*nd,mf,4)
         end if
C------------------
      enddo
...
c            do 41 n = 1,nrm
c               write(l6,'(a,2i3)')
c     &         ' Radial function and derivative for n,l =',n,l
c               ix = 5
c               write(l6,'(5f15.8)') (rnl(n,l,ih),ih=1,ix)
c               write(l6,'(5f15.8)') (rnl1(n,l,ih),ih=1,ix)
c   41       continue
c            do 50 n2 = 1,nrm
c            do 50 n1 = n2,nrm
c               s1 = zero
...
c              do ih = 0,ngh
c                  rr = xh(ih)**2
c                  s0 = rnl(n1,l,ih)*rnl(n2,l,ih)
c                  s1 = s1 + s0
c                  s2 = s2 + rr*s0
c                  s3 = s3 + (rnl1(n1,l,ih)*rnl1(n2,l,ih)
c     &                       + rnl1(n1,l,ih)*rnl(n2,l,ih)/xh(ih)
c     &                       + rnl(n1,l,ih)*rnl1(n2,l,ih)/xh(ih)
c     &                       + s0*(1+l*(l+1))/rr)
c		  if (l.eq.0) then
c		     sb = sb + rnb(n1,ih)*rnb(n2,ih)*rr*wh(ih)
c                  endif
c               enddo
c               write(l6,'(a,2i3,4f12.8)')
c     &                  ' RNL(n,l) test ',n1,n2,s1,s2,s3,sb
c   50       continue
c   40    continue
c         write(l6,'(/,80(1h*))')
c      endif
     ...
     &           nucnam,nama,'  N = ',nama - npro,'  Z = ',npro
           ...
           do it=1,2
             do ib=1,nb
                  ...
                  ie = im+nf-1
                  do 20 k=im,ie
                    !write(l6,*)'nf', nf, 'im', im, 'ie', ie, 'k', k
                    if (eecan(k,it).gt.13.)               goto 20
                    ipar = ipb(ib)
                    e1 = eecan(k,it) !energy
                   ! write(l6,*)e1
                    write(l6,104) 2*ijb(ib)-1, tp(ipar), e1
...
c      write(lplo,111)  it,k1,2*ijb(ib)-1,tp(ip),eecan(k,it)
c111   format(/,3i3,'/2',a1,f10.3)     
c         write(lplo,'(/,a,3i3)') ' wavefunction f(r)',it,ib,k
c         if (lpr)  then
c          if (iplot.eq.1 .or. iplot.eq.2)
c     +     write(l6,111) it,k1,2*ijb(ib)-1,tp(ip),eecan(k,it)
claudio 04/2000
c          if (iplot.eq.1) write(l6,'(1x,2i3)') nf,ng
claudio 04/2000
c         endif
c         r = zero
c         s = zero
...
c         endif
         ...
         s1 = zero
         s2 = zero
         do ist = 0,mxpl + 0.5*mxpl
            !write(l6,*)'ib',ib
           call rwave(it,ib,k,az,r,f,g,ih)
